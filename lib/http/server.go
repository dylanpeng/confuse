...
// Running check server is running or not
...
func (s *Server) Recovery(ctx *gin.Context) {
	defer func() {
		if err := recover(); err != nil {
			...
			request := fmt.Sprintf("%s | %s %s | %s | %s", ctx.ClientIP(), ctx.Request.Method, ctx.Request.URL.RequestURI(),
				ctx.Request.UserAgent(), s.router.GetIdentifier(ctx))
			s.logger.Errorf(fmt.Sprintf("recovered panic:\nRequest: %s\nTrace: %s\n%s", request, err, stacktrace))
...
// CrossDomain cross domain middleware
...
func (s *Server) Start() {
	go func() {
		...
		// start server. https server setting certificate and key file path
...
func NewServer(c *Config, r Router, l *logger.Logger) *Server {
	...
	gin.DefaultWriter = l.GetWriter()
	gin.DefaultErrorWriter = l.GetWriter()
