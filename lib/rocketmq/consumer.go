...
func (c *Consumer) startWork(index int) {
	defer c.wg.Done()
	...
	for body := range c.msgQueue {
		if err := c.handler(body); err != nil {
			...
			c.logger.Debugf("handle rocketmq message | message: %s | index: %d", string(body), index)
		}
	}

	c.logger.Debugf("worker index: %d stop", index)
}
